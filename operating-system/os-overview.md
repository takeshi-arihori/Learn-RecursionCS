...
## プロセスツリー
コンピュータ上で動作するプログラムのインスタンスで、生成時に独立のメモリ空間が割り当てられる。
プロセスには親子関係があり、ほかのプロセスを生成する際に発生する。(プロセスが別のプロセスを生成した際、元のプロセスを「親プロセス」と呼ぶ)

## スレッド
親プロセスと同じメモリ空間を共有し、同じプロセス内で複数のタスクを平行に処理するための軽量な実行単位。

![os-thread](https://github.com/user-attachments/assets/63d1e9a1-5345-4a20-8bca-018d69ccb128)

### スレッドの構成要素
- スレッドID: 各スレッドの一意の識別子で、オペレーティングシステムやアプリケーションが特定のスレッドを識別するために使用。
- プログラムカウンタ: 次に実行される命令のアドレスを保持。
- レジスタセット: スレッドが使用するCPUのレジスタの内容を保持。
- スタック: 一時的なデータの格納や関数呼び出しの際の戻りアドレスの保存に使用されるメモリ領域。

### マルチスレッド
1つのプロセス内で複数のスレッドを持つこと。利点は、CPUのプロセッサを並列で使用できるため、計算処理中でもほかの操作を行うことが可能となる。
親プロセスのメモリ空間を共有しているため、プロセス間通知や共有メモリの設定なしにデータの共有も可能。
マルチプロセス、マルチスレッドには、並行処理、並列処理といった概念がある。
![single-thread](https://github.com/user-attachments/assets/827d6d2d-cc16-4d98-9ce9-ec7dc4533b7a)

#### 並行処理
一つのプロセッサで複数のタスク(プロセス)を一度に処理する技術。順番に素早く切り替えることで、複数のタスクが同時に実行されているように見せることが可能。

![concurrent-processing](https://github.com/user-attachments/assets/281b3268-6753-457b-ab06-65c0f90109d3)


#### 並列処理
複数のコアを持つプロセッサで、同時に複数の処理を行う技術。同じタスクを分割して複数のコアで同時に処理することで、全体の実行時間を大幅に短縮することができる。

## リソース管理
### プロセス管理
プロセスの実行 -> CPU、メモリ、I/Oデバイスなど多くのリソースが必要。そしてそのリソースは有限のためリソースの競合や不足が生じる可能性がある。
そのため、OSはプロセスをスケジューリングし、それぞれのプロセスに優先度を割り当てることで、リソースのアクセスを調整する。また、プロセスを一時的に停止したり再開したりする機能や、プロセス間でデータの通信をサポートする役割もある。

### メモリ管理
メインメモリは、CPUが直接参照できる高速な記憶領域であり、OSはこのメインメモリの管理を行う。OSは、メモリの各部分がどのプロセスによって利用されているかの監視、メモリ領域の確保・解放、どのデータやプロセスがメモリ上にロードまたアンロードされるかの判断などの役割を果たす。
メインメモリの更新速度CPUのクロック速度よりも遅いため、メモリのアクセス速度がシステム全体の処理能力のボトルネックになることがある。その問題を解決するためにキャッシュという高速なメモリの一部を使用してメインメモリとCPUの間の速度差を埋めている。

![cache-coherency](https://github.com/user-attachments/assets/5a0b850f-6cb6-4390-8587-5ed497e5d947)
